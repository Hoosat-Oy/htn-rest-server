...
class HtndClient(object):
    def __init__(self, kaspad_host, htnd_port):
    ...
    async def request(self, command, params=None, timeout=5):
        with HtndThread(self.kaspad_host, self.htnd_port) as t:
            return await t.request(command, params, wait_for_response=True, timeout=timeout)

    async def notify(self, command, params, callback):
        t = HtndThread(self.kaspad_host, self.htnd_port, async_thread=True)
